...
public class SalvoController {
    ...
    /*
    este metodo es para dar el el mensaje del 401 en caso de que:
    no se encuentre algun dato, no exista algun usuario o para cuando crean un nuevo usuario exitosamente
     */
    ...
    public ResponseEntity<Map<String, Object>> createGame(Authentication authentication) {
        ...
        if (player == null) {
            //si el usuario no esta autenticado tirar error
    ...
    public ResponseEntity<Map<String, Object>> joinGame(@PathVariable Long gameId, Authentication authentication) {
        ...
        if (!game.isPresent()) { //isPresent viene de la clase "Optional", en este caso si no esta presente
            return new ResponseEntity<>(makeMap("error", "CONFLICTO"), HttpStatus.CONFLICT);
        }
        if (game.get().getGame_players().size() > 1) { //para que no entren jugadores de mas
    ...
    /*
    metodo para avisar errores:
    .en caso de que otro usario se haya colado
    .en caso de salto de turnos
    .desconexion o ausensia de un player
    .el oponente no coloco sus barcos
     */
    ...
    public ResponseEntity<Map<String, Object>> fireSalvos(@PathVariable Long gamePlayerId, Authentication authentication, @RequestBody Salvo salvo) {
        ...
        if (opponentGamePlayer.get().getShips().size()==0){
            return new ResponseEntity<>(makeMap("error", "oponente aun no posiciona las naves"), HttpStatus.FORBIDDEN);
        }


        gamePlayer.get().addSalvo(salvo);
        //determina los puntos que se le da los players despues de terminar la partida
        if ( gamePlayer != null ) {
